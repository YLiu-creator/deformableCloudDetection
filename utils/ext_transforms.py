...
class ExtRandomHorizontalFlip(object):
    ...
    def __call__(self, img, lbl):
        ...
        if random.random() < self.p:
            # return F.hflip(img), F.hflip(lbl)
            img = np.asarray([np.fliplr(img[0]),np.fliplr(img[1]),np.fliplr(img[2]),np.fliplr(img[3])])
            lbl = np.fliplr(lbl)
            return img, lbl
        return img, lbl
...
class ExtRandomVerticalFlip(object):
    ...
    def __call__(self, img, lbl):
        ...
        """
        if random.random() < self.p:
            img = np.asarray([np.flipud(img[0]), np.flipud(img[1]), np.flipud(img[2]),np.flipud(img[3])])
            lbl = np.flipud(lbl)
            return img, lbl
        return img, lbl
...
class ExtNormalize(object):
    ...
    def __init__(self, train_max, train_min, mean, std):
        self.mean = mean
        self.std = std
    ...
    def __call__(self, tensor, lbl):
        ...
        """
        rsData = tensor
        for idx in range(rsData.shape[0]):
            temp = rsData[idx,:,:] - self.train_min[idx]
            rsData[idx,:,:] = temp/( self.train_max[idx]- self.train_min[idx])

        return F.normalize(rsData, self.mean, self.std), lbl
