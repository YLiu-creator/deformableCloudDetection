...
from .utils import segmentation_stats, segmentation_stats_gf1, segmentation_stats_mask, segmentation_stats_phase, get_optimizer, get_criterion
...
class FeedForwardSegmentation(BaseModel):
    ...
    def initialize(self, opts, **kwargs):
        ...
        self.net = get_network(name=opts.model_type, n_classes=opts.output_nc,
    ...
    def backward(self):
        self.loss_S = self.criterion(self.prediction, self.target.long())
    ...
    def get_segmentation_stats(self):
        self.seg_scores, self.dice_score = segmentation_stats(self.prediction, self.target.long())
        seg_stats = [('Overall_Acc', self.seg_scores['overall_acc']), ('Mean_IOU', self.seg_scores['mean_iou']),
                     ('freq_w_acc', self.seg_scores['freq_w_acc']), ('Kappa', self.seg_scores['Kappa']),
                     ('Prodecer Acc', self.seg_scores['Prodecer Acc']), ('User Acc', self.seg_scores['User Acc']),
                     ('class1 Acc', self.seg_scores['class1 Acc']), ('class2 Acc', self.seg_scores['class2 Acc'])]
    ...
    def get_segmentation_stats_gf1(self):
        self.seg_scores, self.dice_score = segmentation_stats_gf1(self.prediction, self.target.long())
        seg_stats = [('Overall_Acc', self.seg_scores['overall_acc']),  ('KSS', self.seg_scores['KSS']),
                     ('PODclr', self.seg_scores['PODclr']), ('PODcld', self.seg_scores['PODcld']),
                     ('FARclr', self.seg_scores['FARclr']), ('FARcld', self.seg_scores['FARcld']),
                     ('Mean_IOU', self.seg_scores['mean_iou']), ('Kappa', self.seg_scores['Kappa']),
                     ('Prodecer Acc', self.seg_scores['Prodecer Acc']), ('User Acc', self.seg_scores['User Acc']),
                     ('clear Acc', self.seg_scores['class1 Acc']), ('cloud Acc', self.seg_scores['class2 Acc'])]

        # for class_id in range(self.dice_score.size):
        #     seg_stats.append(('Class_{}'.format(class_id), self.dice_score[class_id]))
        return OrderedDict(seg_stats)

    def get_segmentation_stats_mask(self):
        self.seg_scores, self.dice_score = segmentation_stats_mask(self.prediction, self.target.long())
        seg_stats = [('Overall_Acc', self.seg_scores['overall_acc']), ('Mean_IOU', self.seg_scores['mean_iou']),
                     ('Kappa', self.seg_scores['Kappa']),('Prodecer Acc', self.seg_scores['Prodecer Acc']),
                     ('User Acc', self.seg_scores['User Acc']), ('class1 Acc', self.seg_scores['class1 Acc']),
                     ('class2 Acc', self.seg_scores['class2 Acc'])]

        # for class_id in range(self.dice_score.size):
        #     seg_stats.append(('Class_{}'.format(class_id), self.dice_score[class_id]))
        return OrderedDict(seg_stats)

    def get_segmentation_stats_phase(self):
        self.seg_scores, self.dice_score = segmentation_stats_phase(self.prediction, self.target.long())
        seg_stats = [('Overall_Acc', self.seg_scores['overall_acc']), ('Mean_IOU', self.seg_scores['mean_iou']),
                     ('Kappa', self.seg_scores['Kappa']),('Prodecer Acc', self.seg_scores['Prodecer Acc']),
                     ('User Acc', self.seg_scores['User Acc']), ('class1 Acc', self.seg_scores['class1 Acc']),
                     ('class2 Acc', self.seg_scores['class2 Acc']), ('class3 Acc', self.seg_scores['class3 Acc']),
                     ('class4 Acc', self.seg_scores['class4 Acc'])]
