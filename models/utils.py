...
from utils.metrics import segmentation_scores, segmentation_scores_gf1
from utils.metrics import segmentation_scores_mask,segmentation_scores_phase,dice_score_list
...
def segmentation_stats_mask(pred_seg, target):
    n_classes = pred_seg.size(1)
    pred_lbls = pred_seg.data.max(1)[1].cpu().numpy()
    gt = np.squeeze(target.data.cpu().numpy())
    gts, preds = [], []
    ...
    iou = segmentation_scores_mask(gts, preds, n_class=n_classes)
    dice = dice_score_list(gts, preds, n_class=n_classes)

    return iou,dice

def segmentation_stats_phase(pred_seg, target):
    n_classes = pred_seg.size(1)
    pred_lbls = pred_seg.data.max(1)[1].cpu().numpy()
    gt = np.squeeze(target.data.cpu().numpy())
    gts, preds = [], []
    ...
    iou = segmentation_scores_phase(gts, preds, n_class=n_classes)
    dice = dice_score_list(gts, preds, n_class=n_classes)
